...
import { removerCollaborator, updateDocumentAccess } from '@/lib/actions/room.actions';
...
const Collaborator = ({ roomId, email, creatorId, collaborator, user }: CollaboratorProps) => {
    ...
    const removeCollaboratorHandler = async (email: string) => {
        setLoading(true);
        await removerCollaborator({ roomId, email });
        setLoading(false);
    };
    return (
            ...
            <div className='flex gap-2'>
                ...
                <div>
                    <p className='line-clamp-1 text-sm font-semibold leading-4 text-white'>{collaborator.name}
                        <span className='text-10-regular pl-2 text-blue-400'>{loading && 'updating...'}</span>
                    </p>
